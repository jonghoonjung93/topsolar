      ...
      for retry in range(5): # 수치가 0일 경우 최대 5번까지 반복
        ...
        while count < WAIT_CNT:
          ...
          if today_kWh[i] == '':  # today_kW1 데이타가 null 이면 while 문을 다시 시도
            ...
            # 당일 수치가 0이거나 당월 수치가 0, 1, 2일 경우 재작업 로직 (단, count가 WAIT_CNT에 도달하기 전이라도 이상 수치면 재시도 대상)
...
def fetch_today_kp():
  ...
  if flag:
    ...
    try:
        ...
        if today_smp == 0:
            printL(f"smp 단가가 0이므로 임시값 111 사용")
            today_smp = 111
    ...
    try:
        ...
        # 수치가 0인 경우 임시값 70000 사용
        if today_rec == 0:
            printL(f"rec 단가가 0이므로 임시값 70000 사용")
            today_rec = 70000
